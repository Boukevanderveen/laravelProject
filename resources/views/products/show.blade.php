                ...
                <div class="col-6">
                    <form action="{{ route('products.add.to.cart') }}" method="POST">
                        ...
                        @if($product->stock > 0)
                            ...
                            <p>Categorieën: @foreach($product->categories as $category){{$category->name}}@endforeach</p>
                            
                            @foreach($product->attributevalues as $attributevalue)<b>{{$attributevalue->Attribute->name}}:</b> 
