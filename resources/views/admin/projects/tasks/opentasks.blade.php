        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    @elseif(isset($member))
                    <option hidden>{{$member->name}}:</option>
                    @else
                    <option hidden>Sorteer:</option>
                    @endif
                    <option disabled><b>Op status:</b></option>
                    @foreach($statuses as $status)
                    <option type="button" value="{{ route('admin.projects.tasks.tasksopensortstatus', [$project, $status]) }}">{{$status->name}}</option>
                    @endforeach
                    <option disabled><b>Op medewerker:</b></option>
                    @foreach($project->users as $member)
                    <option type="button" value="{{ route('admin.projects.tasks.tasksopensortmember', [$project, $member]) }}">{{$member->name}}</option>
                    @endforeach
                </select>
                </tr>
            </thead>
                    ...
                    <tr>
                        ...
                        <td> 
                            <form method="post" action="{{ route('admin.projects.tasks.tasksdestroy', [$project, $task]) }}"> @csrf @method('delete')
                                <td class="text-end">
                                    <a href="{{ route('admin.projects.tasks.taskscomplete', [$project, $task]) }}"><button type="button"
